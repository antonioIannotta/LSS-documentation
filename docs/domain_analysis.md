...
| Parking slot marker | A marker used to identify the position of a parking slot in a map, it can be be of three different colours: green if the parking slot is free, orange if it will be free in ten minutes, red if none of the other cases apply. |
| Parking slot status | The current status of the parking slot: occupied or not occupied. |
| Range of interest | The distance selected by the user in the map to determine the area in which they want to see the parking slots. |
